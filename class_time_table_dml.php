...
function class_time_table_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	<script>
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#school-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						...
						success: function(resp){
							...
							if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=Faculties_view_parent]').hide(); }else{ $j('.btn[id=Faculties_view_parent]').show(); }
				...
				if(e.added.id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=Faculties_view_parent]').hide(); }else{ $j('.btn[id=Faculties_view_parent]').show(); }
	...
	$lookup_fields = array(  'school' => array('Faculties', 'School'), 'department' => array('departments', 'Department'));
